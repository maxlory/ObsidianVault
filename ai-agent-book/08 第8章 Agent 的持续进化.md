...
part: "正文"
chapter: 8
source: https://github.com/bojieli/ai-agent-book/blob/main/book/chapter8.md
license: Apache-2.0
tags:
  - ai-agent-book
  - AI-Agent
...
>
> 配套实现见 [`trajectory-verifier`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/trajectory-verifier/)，默认使用可离线复现的质量 Judge；使用 `--judge llm` 可运行已经实现的真实 LLM 验证器。
...
>
> 配套实现见 [`gaia-experience`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/gaia-experience/)。`demo_documents.py` 默认离线运行，使用 `--extractor llm` 可由真实 LLM 提出跨轨迹经验候选。
...
>
> 配套实现见 [`prompt-auto-optimization`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/prompt-auto-optimization/)。离线测试覆盖诊断与发布门槛，`--quick` 则会真实调用任务 Agent、LLM Judge 和 Coding Agent。
...
>
> 配套实现见 [`browser-use-rpa`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/browser-use-rpa/)，同时提供确定性状态机演示和调用真实浏览器 Agent 的运行路径。
...
>
> 配套实现见 [`self-modifying-agent`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/self-modifying-agent/)，可选择确定性候选生成器或真实 LLM Coding Agent，两条路径共用同一发布门槛。
...
>
> 配套实现见 [`self-evolution-eval`](https://github.com/bojieli/ai-agent-book/tree/main/chapter8/self-evolution-eval/)，默认比较可更新、只追加和静态三种参考 Agent；使用 `--profile llm` 可让真实 LLM 经历同一长期任务流。
