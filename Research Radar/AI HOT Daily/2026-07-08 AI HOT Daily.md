...
# 2026-07-08 AI HOT Daily

## AI HOT 官方日报

### 模型发布/更新

> [!info]+ **可延后 / 71** | Meta Superintelligence Labs 推出 Muse Image 和 Muse Video
> **标题**：Meta Superintelligence Labs 推出 Muse Image 和 Muse Video
> **原文链接**：🔗 [打开原文](https://x.com/AIatMeta/status/2074577662840832382)
> **source**：AI HOT Daily / X：AI at Meta (@AIatMeta)
> **kind**：`model`
...
> [!info]+ **可延后 / 66** | MIRA：可玩多人世界模型，20 FPS实时生成“火箭联盟的梦”
...
> [!info]+ **可延后 / 66** | Grok Imagine 更新：支持 15 秒视频
...
> [!info]+ **可延后 / 66** | Claude Cowork 向移动端和网页端开放
...
> [!info]+ **可延后 / 66** | NotebookLM短视频概览正式上线
...
### matches topics: claude code, codex, obsidian, mcp; high-value terms: mcp, codex, claude code, api
...
### matches topics: agent, agents, mcp; high-value terms: agent, agents, mcp, api

> [!info]+ **今日必须看 / 100** | Gemini API Managed Agents 新增后台执行、远程 MCP 与自定义函数等能力
...
> [!info]+ **可延后 / 74** | Hugging Face Storage 成为 SkyPilot 一级后端：零出站费跨云存储
> **标题**：Hugging Face Storage 成为 SkyPilot 一级后端：零出站费跨云存储
> **原文链接**：🔗 [打开原文](https://huggingface.co/blog/skypilot-hf-storage)
...
> [!info]+ **今日必须看 / 96** | Claude Code v2.1.203 发布
> **标题**：Claude Code v2.1.203 发布
> **原文链接**：🔗 [打开原文](https://github.com/anthropics/claude-code/releases/tag/v2.1.203)
...
> [!info]+ **今日必须看 / 75** | Weblica：面向视觉网页智能体的可扩展可复现训练环境
> **标题**：Weblica：面向视觉网页智能体的可扩展可复现训练环境
> **原文链接**：🔗 [打开原文](https://machinelearning.apple.com/research/weblica-visual-web-agents)
...
> [!info]+ **可延后 / 64** | Ethan Mollick：开放权重模型供给难持续
...
> [!info]+ **今日必须看 / 80** | 微软为降成本在Copilot中用自研MAI模型替换OpenAI和Anthropic模型
...
> [!info]+ **今日必须看 / 80** | FDE爆发：AI公司12个月承诺97.5亿美元部署工程
> **标题**：FDE爆发：AI公司12个月承诺97.5亿美元部署工程
> **原文链接**：🔗 [打开原文](https://www.tomtunguz.com/the-10b-fde-boom)
...
> [!info]+ **可延后 / 68** | 苹果研究：单个神经元即可绕过大型语言模型的安全对齐
> **标题**：苹果研究：单个神经元即可绕过大型语言模型的安全对齐
> **原文链接**：🔗 [打开原文](https://machinelearning.apple.com/research/single-neuron-safety-alignment)
> **source**：AI HOT Daily / Apple Machine Learning Research（RSS）
> **kind**：`paper`
...
> [!info]+ **可延后 / 68** | DynaMiCS：带性能约束的大语言模型动态混合微调
> **标题**：DynaMiCS：带性能约束的大语言模型动态混合微调
> **原文链接**：🔗 [打开原文](https://machinelearning.apple.com/research/dynamics-fine-tuning-llms)
> **source**：AI HOT Daily / Apple Machine Learning Research（RSS）
> **kind**：`paper`
> **reason**：AI HOT official daily section: 论文研究
> **follow_up**：阅读摘要和方法，判断是否需要建立永久论文笔记。
...
### matches topics: agent, codex, mcp; high-value terms: agent, mcp, codex

> [!info]+ **今日必须看 / 100** | Elvis Saravia 通过 HITL 和 DialAgent 提升 agentic loops 可靠性
...
> [!info]+ **可延后 / 64** | YC CEO声称每日用AI部署3.7万行代码，开发者审查发现前端代码大量臃肿低效
...
> [!info]+ **可延后 / 64** | Liquid AI 开源 Antidoom：基于最终 Token 偏好优化的推理模型死循环修复方法
...
> [!info]+ **可延后 / 64** | 在网络不稳定的地区，小型AI模型正逐渐普及
...
> [!info]+ **可延后 / 64** | 人们如何使用Claude Cowork
...
> [!info]+ **今日必须看 / 79** | 在 Claude Code 中选择 Claude 模型与努力级别
